CREATE OR REPLACE NONEDITIONABLE PROCEDURE GuidesProcedure IS
---FIELDS FOR ADDING OPENDAY AND GUESTLECTURE
 ...
 PRAGMA EXCEPTION_INIT(cannotInsert, -9880);
 message varchar2(100);
 
 ---CONSTANT FIELDS
 num_consulting_treshold CONSTANT number := '10';
     ...
     ELSE --IF YES THEN REGISTER ALL GRADUETED STUDENTS TO THE NEW LECTURE
       ...
       LOOP --FOR EVERY RECORD IN THE CURSOR
        ...
        IF(Guide.NUMCONSULTS > num_consulting_treshold) THEN --ONLY IF IT'S OVER THE TRESHOLD
          ...
          IF(SQL%NOTFOUND) THEN
            message := 'Couldnt insert TO interested this guide, ID: ' || Guide.gpersonid;
            RAISE cannotInsert; --IF NOT THEN RAISE EXCEPTION
          END IF;
          ...
          IF(SQL%NOTFOUND) THEN
            message := 'Couldnt insert to participant in this guide, ID: ' || Guide.gpersonid;
            RAISE cannotInsert; --IF NOT THEN RAISE EXCEPTION
          END IF;
          ELSE
